...
PLOT_DIR = os.path.join(BASE_DIR, "ML_Report/SVR_Plot")
MODEL_DIR = os.path.join(BASE_DIR, "ML_Report/SVMReg_Trained_Model")

os.makedirs(PLOT_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)


# -------------------- SVMRegressor Missioner --------------------
class SVMRegressor_Missioner(BaseModelConfig):
    """
    Mission-layer class for Support Vector Machine regression workflows.

    This class extends :class:`BaseModelConfig` and provides shared regression
    utilities for SVR-based model trainers. It serves as the mission layer between
    the shared base configuration workflow and concrete SVM regressor model classes.
    ...
    - Evaluate trained SVR pipelines on train and test splits.
    ...
    - SVR-based estimators do not expose ``feature_importances_``, so no
    feature-importance utility is implemented here.
    - Unlike tree-based models, SVR does not provide a structural visualization
    such as tree plots.
    ...
    logic shared across SVM-based regression workflows.
    ...
    def model_evaluation_engine(self) -> Dict[str, Any]:
        """
        Evaluate a trained SVR regression pipeline on train and test datasets.
        ...
        y_true_train = self.Y_train  # DataFrame
        y_true_test = self.Y_test  # DataFrame
        y_pred_train = self.y_train_pred  #  Numpy ndarray
        y_pred_test = self.y_test_pred  # Numpy ndarray
    ...
    # -------------------- Residual scatter plot --------------------
    def plot_svr_regression_diagnostics(
        self,
        dataset: str = "test",
        bins: int = 30,
        alpha: float = 0.6,
        ...
        """
        Generate and save SVR regression diagnostic plots.
        ...
        for col in y_true_plot.columns:
            ...
            plt.title(f"svm_reg | {dataset} | Predicted vs Actual | target={col}")

            out = os.path.join(
                PLOT_DIR, f"svm_reg_{dataset}_pred_vs_actual_{col}_{timestamp}.png"
            )
            ...
            plt.title(f"svm_reg | {dataset} | Residual scatter | target={col}")

            out = os.path.join(
                PLOT_DIR, f"svm_reg_{dataset}_residual_scatter_{col}_{timestamp}.png"
            )
            plt.savefig(out, dpi=150, bbox_inches="tight")
            saved_files.append(out)
            ...
            plt.ylabel("Count")
            plt.title(f"svm_reg | {dataset} | Residual histogram | target={col}")

            # ---------- Saving path ----------
            out = os.path.join(
                PLOT_DIR, f"svm_reg_{dataset}_residual_hist_{col}_{timestamp}.png"
            )
            plt.savefig(out, dpi=150, bbox_inches="tight")
            saved_files.append(out)
    ...
    # -------------------- Save trained model --------------------
    def save_model_joblib(self, filename: str = "svm_regressor.joblib") -> str:
        """
        Save the trained SVR pipeline and related metadata as a joblib artifact.
        ...
        Parameters
        ----------
        filename : str, default="svm_regressor.joblib"
    ...
    def load_model_joblib(cls, filepath: str):
        """
        Load a previously saved SVR model artifact from a joblib file and
        restore it as an ``SVMRegressor_Missioner`` instance.
